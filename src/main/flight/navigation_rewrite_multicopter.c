...
bool adjustMulticopterAltitudeFromRCInput(void)
{
    ...
    if (ABS(rcThrottleAdjustment) > 0) {
        ...
        if (rcThrottleAdjustment > 0) {
            ...
            rcClimbRate = rcThrottleAdjustment * posControl.navConfig->max_manual_climb_rate / (posControl.escAndServoConfig->maxthrottle - altHoldThrottleRCZero - posControl.rcControlsConfig->alt_hold_deadband);
        }
        else {
            // Scaling from minthrottle to altHoldThrottleRCZero
            rcClimbRate = rcThrottleAdjustment * posControl.navConfig->max_manual_climb_rate / (altHoldThrottleRCZero - posControl.escAndServoConfig->minthrottle - posControl.rcControlsConfig->alt_hold_deadband);
        }

        rcClimbRate = constrainf(rcClimbRate, -posControl.navConfig->max_manual_climb_rate, +posControl.navConfig->max_manual_climb_rate);
